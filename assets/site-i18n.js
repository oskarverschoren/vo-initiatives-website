...
(function () {
  ...
  const DICT = {
    en: {
      ...
      "dash.wiz.help.gmail": `Create an app password via myaccount.google.com → Security → App passwords (2-step verification must be on). Your regular password won't work and we never ask for it.`,
      "dash.wiz.help.outlook": `Create an app password via account.microsoft.com → Security → App passwords. Your regular password won't work and we never ask for it.`,
      "dash.wiz.help.ics": `Paste your calendar's secret iCal link (Google Calendar → settings → 'Secret address in iCal format'). Read-only — we can't change anything.`,
    ...
    fr: {
      ...
      "dash.wiz.help.gmail": `Créez un mot de passe d'application via myaccount.google.com → Sécurité → Mots de passe d'application (la validation en deux étapes doit être activée). Votre mot de passe habituel ne fonctionne pas et nous ne le demandons jamais.`,
      "dash.wiz.help.outlook": `Créez un mot de passe d'application via account.microsoft.com → Sécurité → Mots de passe d'application. Votre mot de passe habituel ne fonctionne pas et nous ne le demandons jamais.`,
      "dash.wiz.help.ics": `Collez le lien iCal secret de votre agenda (Google Agenda → paramètres → « Adresse secrète au format iCal »). Lecture seule — nous ne pouvons rien modifier.`,
